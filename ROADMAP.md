...
| 25 | 🚧 in progress | **MOD-IV transactions and market value** — New Jersey's own transaction prices and its own effective tax rate, per municipality and per county, closing the **effective tax rate against market value** that ARCHITECTURE #141 wanted and rejected for lack of "sale prices or equalization ratios the warehouse does not hold". Both exist, keyless, and neither is a MOD-IV column. The transaction half is the **SR1A Sales File** — 663-byte fixed-width records, one archive per year since 2020, carrying `U-N-TYPE`, the 33 `SR-NU-CODE` non-usable categories, and both reported and verified price — not MOD-IV's `SALE_PRICE`/`SALES_CODE`, which this milestone's scoping proved cannot do the job: `SALES_CODE` records how a sale was *investigated* (A/F/Q), and the usability field `SALE-SR1A-UN-CODE` is absent from every field of NJ's ArcGIS publication at any token level (verified 2026-09-19). The tax half is the state's published **General and Effective Tax Rates** (1997–2025), validated against the **Director's Ratio History** rather than derived from it. Both key on the NJ CD code and join through `region_identifiers` with no name matching |
