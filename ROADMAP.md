...
**Shipped: 24, 25, 26, 27, 29. Next: 28.**
...
| 26 | ✅ done | **Current releases** — built 2026-09-23 as 0.22.0. Each dated source now **discovers** its newest release at acquisition and records it in `data/raw/<source>/releases.json`, which every later stage reads rather than probes; the old constants are floors (ARCHITECTURE #206). The first run loaded what hard-coded years had missed: BLS through July 2026 (147 county-months), the 2025 Building Permits file, IRS migration for 2022–23 with a caveat on its changed matching method, and HUD income limits for FY2025 and FY2026. The record keeps a release's **publication date** and, where it differs, the day it takes effect: HUD's FY2027 Fair Market Rents were already published and wait as pending until 2026-10-01. Verifying that run exposed a cache that answered BLS's new twenty-year window with the old one; a cached copy now answers only the request it was fetched with (#214). TIGER stays pinned on purpose. **The mortgage rate** (#207): today's cost card is priced at Freddie Mac's weekly benchmark — 6.95% for the week of 2026-09-17, where the monthly figure it replaced read 6.67% — and the monthly averages keep the history. A month in progress ends on its last observed week instead of being labelled provisional: the period already carries the fact, and a status column would have changed every fact row for one series' edge. **Audits:** MOD-IV figures are dated by the tax year NJOGIS joined, 2024, not by when a county last redrew its parcels — 4,067 figures redated through migration 0014 (#208), with the 2025 tax year expected in February 2027; the old MOD-IV layer was retired rather than put behind a token (#212, correcting #191); SR1A's archive year runs July to June, which the models already handled (#209); and NJ's General and Effective rates are discovered together and must name the same year. FHFA's county and ZIP indexes proved reachable, so the caveat saying otherwise was corrected, and loading them is Milestone 32's question (#210). **Every source records a fallback**, which `hip check-config` enforces, and README has the procedure for a source that stops answering (#211). **The models:** Qwen 3.7 Plus left the preference list, and `hip explain --all` now retires readings from any model no longer on it, pinned by a test (#213). **Gemma 4 E4B stays as it was, by measurement** (#215): the copy LM Studio and the project share was already one file; every MLX build is larger than the GGUF; the model run swapped out 184MB; and capping Docker Desktop at 4GB brought the owner's swap down 0.8–0.9GB, so the planned trims were not made and the project stays on Ollama. The county explanations were regenerated after the merge: four models, with Qwen's 21 readings retired |
| 27 | ✅ done | **Refresh reaches the reader** — built 2026-09-26 as 0.23.0. **Where it runs and what it may do, decided with the owner 2026-09-25:** this Mac, under `launchd`, every Friday at 08:00 inside `caffeinate` — not a cloud runner, so the automated screenshots ARCHITECTURE #175 waits on stay blocked. `scripts/scheduled_refresh.py` calls the existing commands in turn rather than lengthening `hip refresh` (#216): a quiet week stops after the refresh (#217); otherwise pack, rebuild, deploy and `check-live` run without asking, and **regenerating readings is the one gated step**, because it is billed — an `ask`/`auto` setting that starts at `ask`, one iCloud file written by `hip refresh-mode` on the Mac or an iPhone Shortcut, with a regenerate-now trigger beside it and `hip explain --dry-run` as the free check (#219). Failures and stale readings reach the owner's phone through Pushover (#218). **A public freshness page**, `/freshness`, keeps each source's newest period, release date, last check and download apart, because *checked today* must never read as *measured today* — with four statuses where this row named six, since *delayed* would need a release calendar nobody keeps and *superseded* is the next page's job (#222); a discovery's publication date now survives refreshes that find nothing newer (#223). **What changed**, `/changes`, shows readers the `fact_revision` rows Milestone 29 records, summarised per refresh and metric because one refresh alone revised 313,536 figures — Zillow restating its history, and Margate City's January rent index cut 46% — with a month still under way kept apart from a correction (#224). `check-live` pins one region per cost-card shape and samples both new pages (#220). **Report a problem** opens a pre-filled GitHub issue from every row of the two tables that list all of a region's metrics — narrower than "every figure": the cards and verdict sentence do not carry it yet (#221). And the [completeness standing check](#the-completeness-standing-check) is `hip completeness`, first run 2026-09-26 (#225) |
...
- ~~**Milestone 27:** where the refresh runs — this Mac or a cloud runner — and what it may
  do without asking.~~ Decided 2026-09-25: this Mac, and everything but regenerating
  readings runs without asking (ARCHITECTURE #216–#219).
...
CHAS; IRS migration; NJ MOD-IV, the SR1A Sales File and NJ's published tax rates. FRED
carries Freddie Mac's weekly 30-year benchmark as well as its monthly average since
Milestone 26. Terms are in `config/sources.yml` and rendered in the site footer. *(Corrected 2026-09-23: this
...
|---|---|---|
| FHFA county and ZIP HPI | Local price trends that do not depend on Zillow — reachable, retested 2026-09-23: all 21 counties from 1975 to 2025, and a 40MB ZIP file (ARCHITECTURE #210) | M32 decides |
...
| **Zillow ZTRAX** | Deed-level transactions and assessments, offered free by application to qualified academic, nonprofit and government researchers through ICPSR (Zillow Research's contact page, read 2026-09-23). Whether this project qualifies, and what its terms let a public site show, is unread. *Corrected 2026-09-23: this row sat under Rejected as discontinued in 2023* |
...
until deployment itself runs without a laptop open. Milestone 27 decided on 2026-09-25 that
the refresh runs on this Mac, and only a runner off this machine unblocks this — so it stays
blocked. Until then the Screenshots section stays empty and
